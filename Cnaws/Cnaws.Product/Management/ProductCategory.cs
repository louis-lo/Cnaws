...
{
    public sealed class ProductCategory : ManagementController
    {
        ...
        public void List(int parentId, int page = 1)
        {
            ...
            {
                if (CheckRight())
                    SetResult(M.ProductCategory.GetPage(DataSource, parentId, page, 10));
            }
        }
        public void Add()
        {
            if (CheckAjax())
            {
                if (CheckRight())
                {
                    if (IsPost)
                    {
                        M.ProductCategory category = new M.ProductCategory()
                        {
                            Name = Request["Name"],
                            Image = Request["Image"],
